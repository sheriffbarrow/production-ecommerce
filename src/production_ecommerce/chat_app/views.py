...
def whoIs(query, sessionID="general"):
    ...
    try:
        response = requests.get('http://api.stackexchange.com/2.2/tags/' +
                                query + '/wikis?site=stackoverflow')
        data = response.json()
...
def results(query, sessionID="general"):
    ...
    query_list = [x for x in query_list if x not in ['posted', 'questions',
                                                     'recently', 'recent', 'display', '', 'in', 'of', 'show']]
    # print(query_list)
    if len(query_list) == 1:
        # print('con 1')
        try:
            response = requests.get(
                'https://api.stackexchange.com/2.2/questions?pagesize=5&order=desc&sort=activity&tagged=' + query_list[0] + '&site=stackoverflow')
            data = response.json()
        ...
        tag = query_list[1]
        try:
            response = requests.get('https://api.stackexchange.com/2.2/questions?pagesize=' +
                                    n + '&order=desc&sort=activity&tagged=' + tag + '&site=stackoverflow')
            data = response.json()
        ...
        # print('con 3')
        query_list = [x for x in query_list if x not in ['which', 'where',
                                                         'whos', 'who\'s' 'is', 'are', 'answered', 'not', 'unanswered', 'for']]
        # print(query_list)
        if len(query_list) == 1:
            try:
                response = requests.get(
...
def bot(request):
    return render(request, "ecommerce/home.html", {'home': 'active', 'chat': 'chat'})
...
def ask(request):
    ...
    if request.method == "POST":
        query = request.POST.get('chat-msg', None)
        ...
        if query.lower() in ['bye', 'quit', 'bbye', 'seeya', 'goodbye']:
            ...
            response = response + '<br/>' + '<h3>Chat Summary:</h3><br/>' + \
                '<br/><br/>'.join(chat_saved)
